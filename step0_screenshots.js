...
const outDir = 'C:\\Users\\Admin\\.gemini\\antigravity\\brain\\cd032d60-6291-4ab1-b6ec-2f23d3838190\\screenshots\\step0_light_redesign';
...
(async () => {
  ...
  for (const pageInfo of pagesToCapture) {
    for (const w of widths) {
      ...
      try {
        ...
        const outFile = path.join(outDir, `before_${pageInfo.name}_${w}.png`);
  ...
  for (const w of widths) {
    ...
    try {
      ...
      const outFile = path.join(outDir, `before_cart_drawer_${w}.png`);
      await page.screenshot({ path: outFile });
      console.log(`Saved screenshot: ${outFile} (${w}px)`);
    } catch (err) {
  ...
  await browser.close();
  console.log('Step 0 screenshots complete!');
})();
